...
- 音视频实时互动直播系统，信令服务器实现，stun/turn服务搭建，音视频互动以及实时聊天功能。传输速率控制，
- RTP、RTCP协议详解
- nodejs 搭建 https 服务，通过 socket.io 建立信令系统
- nodejs做信令处理，数据流是通过 C/C++处理。C/C++会作为 nodejs的一个库，加载起来
...
- 猫狗大战（猫狗分类）
  ...
  **项目思考**：项目使用的 Xception，InceptionV3 和 ResNet50 这三个模型进行了提取特征向量，然后将特征向量直接拼接，忽略了特征之间的位置关系。除了这三个模型，还可以增加更多新的模型，或者使用stacking 的方法进行模型融合，进一步降低方差，提高分类的准确率。（还可以从原始样本上入手，有些图片的分类概率正确，但是不够确定，可以先做一部分处理，然后让模型更加确定。）
